        ...
        if field.data != other.data:
            ...
            if self.message is None:
                self.message = field.gettext(
                    u'Field must be equal to %(other_name)s.')
    ...
    def __call__(self, form, field):
        ...
        if data is None or (self.min is not None and data < self.min) or \
            ...
            if self.message is None:
                ...
                if self.max is None:
                    self.message = field.gettext(
                        u'Number must be greater than %(min)s.')
                elif self.min is None:
                    self.message = field.gettext(
                        u'Number must be less than %(max)s.')
                else:
                    self.message = field.gettext(
                        u'Number must be between %(min)s and %(max)s.')
...
class IPAddress(Regexp):
    ...
    def __init__(self, message=None):
        super(IPAddress, self).__init__(
            r'^([0-9]{1,3}\.){3}[0-9]{1,3}$', message=message)
...
class AnyOf(object):
    ...
    def __call__(self, form, field):
        if field.data not in self.values:
            if self.message is None:
                self.message = field.gettext(
                    u'Invalid value, must be one of: %(values)s.')
...
class NoneOf(object):
    ...
    def __call__(self, form, field):
        if field.data in self.values:
            if self.message is None:
                self.message = field.gettext(
                    u'Invalid value, can\'t be any of: %(values)s.')
